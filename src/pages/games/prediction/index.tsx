...
import { LaterRoundCard } from "./components/LaterRoundCard"
...
import { WinsDrawer } from "./components/WinsDrawer"
import { Flex, HStack, Image, Text, useDisclosure, VStack } from "@chakra-ui/react"
import { useRoundStatus } from "@hooks/games/prediction/query/useRoundStatus"
...
// const initialCandleData: Array<any> = [
//   {
//     time: "2018-10-19",
//     open: 180.34,
//     high: 180.99,
...
const Prediction = () => {
	...
	const [roundStatus, isLoadingRoundStatus] = useRoundStatus()
	const [lastRound, isLoadingLastRound] = useFinishedRound({
		roundId: Number(Number(roundStatus?.live_round?.id) - 1).toString()
	})
	const [penultimateRound, isLoadingPenultimateRound] = useFinishedRound({
		roundId: Number(Number(roundStatus?.live_round?.id) - 2).toString()
	})

	const [wins, isLoadingWins] = useGetWins()
	const previousWins = useRef(wins?.length ?? 0)
	const [showAnimation, setShowAnimation] = useState<boolean>(false)

	useEffect(() => {
		if (isLoadingWins) return
		if (wins && !isLoadingWins) {
			if (previousWins.current < wins.length) {
				setShowAnimation(true)
				previousWins.current = wins.length
			} else {
	...
	useMemo(() => {
		timerRef.current = dayjs(Number(roundStatus?.live_round?.close_time) / 1e6)
	}, [roundStatus])
	...
	return (
		...
		>
			<Helmet>
				<title>Prediction | Electron</title>
			</Helmet>
			{showAnimation && <WinAnimation />}
			<Flex
				align="center"
			...
			>
				...
				<HStack>
					<WinButton onOpen={onOpen} />
					<GameTimer time={formattedResponse} />
				</HStack>
			</Flex>
			<Swiper
				breakpoints={{
			...
			>
				...
				>
					<PreviousRoundCard isLoading={isLoadingPenultimateRound} roundInfo={penultimateRound!} />
				</SwiperSlide>
				<SwiperSlide
					style={{
						display: "flex",
				...
				>
					<PreviousRoundCard isLoading={isLoadingLastRound} roundInfo={lastRound!} />
				</SwiperSlide>
				<SwiperSlide
					style={{
						display: "flex",
				...
				>
					<LiveRoundCard
						currentTime={roundStatus?.current_time}
						isLoading={isLoadingRoundStatus}
						roundInfo={roundStatus?.live_round!}
					/>
				</SwiperSlide>
				<SwiperSlide
					style={{
				...
				>
					<NextRoundCard roundInfo={roundStatus?.bidding_round!} />
				</SwiperSlide>
				<SwiperSlide
					style={{
						display: "flex",
				...
				>
					<LaterRoundCard
						id={Number(roundStatus?.bidding_round?.id) + 1}
						time={formattedResponse}
					/>
				</SwiperSlide>
			</Swiper>
			{wins && <WinsDrawer isOpen={isOpen} onClose={onClose} wonRounds={wins} />}
			<Flex
				bg="slate.700"
				bottom="1.5rem"
				h="3rem"
				...
				shadow="md"
			>
				<HStack justify="start" spacing={1} w="full">
					<Image src="/assets/tokens/ntrn.png" w="2rem" />
