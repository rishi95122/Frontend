...
const DymensionIBCtransfer = () => {
	const { address, isWalletConnected } = useChain("dymension")
		...
		const fetchPercentage = async (): Promise<void> => {
			try {
				const response = await axios.get<AddressData[]>(
					"https://raw.githubusercontent.com/Electron-Protocol/airdrop/main/Dymension/Dymension_percentage_IBCtransfer.json"
				)
...
export default DymensionIBCtransfer
